...
6. **Always read relevant spec before implementing.**
7. **Reference specs with: `@specs/features/task-crud.md`**
8. **Update specs if requirements change.**
...
**File:** `speckit.plan`  
Includes: Component breakdown, APIs & schema diagrams, Service boundaries, System responsibilities, High-level sequencing. All architectural output MUST be generated from the Specify file.

### **4. Tasks (BREAKDOWN — Atomic, Testable Work Units)**
**File:** `speckit.tasks`  
Each Task must contain: Task ID, Clear description, Preconditions, Expected outputs, Artifacts to modify, Links back to Specify + Plan sections. Agents **implement only what these tasks define**.
