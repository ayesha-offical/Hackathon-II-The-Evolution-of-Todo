...
- **[ID]**: Task identifier in format T-001, T-002, etc. (referenced in git commits)
...
Each task MUST produce code that includes traceability references:
...
Task ID: T-001
Specification: FR-001 - System MUST support task creation with metadata
"""
```

**In git commits** (format per Constitution Principle VIII):
```
...
**Coverage Requirement**: All code from this task MUST contribute to 80% minimum coverage target (measured via `pytest-cov`)
